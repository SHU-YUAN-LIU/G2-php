...
try {
    ...
    if (!isset($donateNo) || $donateNo > 0) {
        $sql = "update member set point= point + :donate_point where member_no=:member_no";
        $member = $pdo->prepare($sql);
        $member->bindValue(":member_no", $donateNo);
        $member->bindValue(":donate_point", $donatePoint);
        $member->execute();
    }
